...
\begin{center}
  {\small U.S. Provisional Patent Application}\\[6pt]
...
This provisional patent application is the foundational disclosure for a series
of three related patent applications. One or more nonprovisional patent
applications claiming the benefit of this provisional application pursuant to
35~U.S.C.\ \S~119(e) will be filed within twelve months of the filing date of
this application. Such nonprovisional applications are expected to include,
without limitation: an application directed to a system and method for federated
retrieval-augmented generation over structurally derived heterogeneous knowledge
graphs; and an application directed to fractal forest visualization of compiled
knowledge graphs with temporal animation and federated query illumination. Each
such nonprovisional application will incorporate the disclosure of this
...
\textbf{FIG.~1} is a conceptual mapping diagram illustrating the
...
\textbf{FIG.~2} is a phase diagram illustrating the separation of the compilation
...
\textbf{FIG.~3} is a data structure diagram illustrating the stable node
...
\textbf{FIG.~5} is a comparison diagram illustrating the semantic search failure
...
\textbf{FIG.~7} is a flow diagram illustrating the three-pass abstract syntax
tree compiler for Python source code, showing the structural extraction pass
(Pass~1), the call-graph extraction pass (Pass~2), the data-flow extraction
pass (Pass~3) with its \texttt{@property} pre-scan, and the post-build symbol
resolution step that emits \texttt{RESOLVES\_TO} edges.
...
Referring to FIG.~\ref{fig:1}, the ontology-determinism principle holds that for
...
Referring to FIG.~\ref{fig:2}, the present invention separates knowledge graph
...
In a representative embodiment, the present invention was applied to a corpus of
78 public-domain literary works spanning 9 genres (GutenbergKG), producing
445,486 nodes and 4,525,716 edges. Build times on Apple M-series hardware ranged
from approximately 4.5 seconds for short works to approximately 30 seconds for
novel-length works, with the full 78-book corpus buildable incrementally. Each
subsequent query across the full corpus executes in bounded milliseconds
independent of corpus size. In a second representative embodiment, the complete
diary of Samuel Pepys (7,282 entries, 1660--1669) was compiled into a temporal
knowledge graph in approximately three minutes end-to-end on the same hardware,
after which any entry or thematic query executes in milliseconds.

% ---
\subsection{III.\quad The Stable Node Addressing Scheme}

Referring to FIG.~\ref{fig:3}, the present invention assigns each node in the
...
\begin{figure}[H]
  ...
  \caption{The stable node addressing scheme. Each node identifier encodes kind
    ...
    Example: \texttt{m:src/kg\_rag/orchestrator.py:KGRAG.query} identifies the
...
function, \texttt{cls} for class, \texttt{mod} for module, \texttt{m} for
method, \texttt{sym} for unresolved symbol stub), \texttt{path} is the
repository-relative path to the source file, and \texttt{qualified} is the
dot-qualified name of the entity within the file. For example,
\texttt{m:src/kg\_rag/orchestrator.py:KGRAG.query} is the stable identifier of
the \texttt{query} method of the \texttt{KGRAG} class defined in the file
\texttt{src/kg\_rag/orchestrator.py}. This identifier encodes the full
provenance of the entity---its type, location, and qualified name---in a single
string.
...
Referring to FIG.~\ref{fig:5}, the structural traversal addresses the
...
% ---
\subsection{VI.\quad The Three-Pass AST Compiler for Python Source Code}

Referring to FIG.~\ref{fig:7}, the Python source code compiler is described in
detail as the primary preferred embodiment of the build-phase parser. The
compiler operates in three sequential passes over the abstract syntax tree of
each source file, followed by a post-build symbol resolution step applied once
across the full compiled graph.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig7.png}
  \caption{The three-pass AST compiler for Python source code. Pass~1
    (structural extraction) emits module, class, function, and method node
    records with \texttt{CONTAINS}, \texttt{IMPORTS}, and \texttt{INHERITS}
    edges. Pass~2 (call-graph extraction) emits \texttt{CALLS} edges from
    calling nodes to resolved or stub destinations. Pass~3 (data-flow
    extraction) emits \texttt{READS}, \texttt{WRITES}, and
    \texttt{ATTR\_ACCESS} edges via a scope-tracking visitor, including
    \texttt{CALLS} edges for \texttt{@property} accesses discovered in a
    pre-scan. A post-build symbol resolution step emits \texttt{RESOLVES\_TO}
    edges from call-site stubs to first-party definitions.}
  \label{fig:7}
\end{figure}

\subsubsection{VI.A\quad First Pass: Structural Extraction}

The first pass traverses the top-level body of the module (not a full
\texttt{ast.walk}) of every \texttt{.py} file in the source repository.
The traversal visits the following AST statement types:
\texttt{ClassDef}, \texttt{FunctionDef}, \texttt{AsyncFunctionDef},
\texttt{Import}, and \texttt{ImportFrom}. For each visited node, the compiler
emits:
...
or symbol, with the import path and alias stored as edge evidence. A supplemental
\texttt{ast.walk} over the full tree extends the import alias map with imports
that appear inside function bodies, ensuring call resolution in Pass~2 can
resolve symbols imported locally.
...
\subsubsection{VI.B\quad Second Pass: Call Graph Extraction}

The second pass constructs a parent-map over the full abstract syntax tree by a
single \texttt{ast.walk}, then performs a second \texttt{ast.walk} visiting
every \texttt{Call} node. For each \texttt{Call} node, the compiler locates the
nearest enclosing \texttt{FunctionDef} or \texttt{AsyncFunctionDef} via the
parent map, resolves the callee expression to a qualified name using the
following priority order: (1)~first-party module locals from Pass~1; (2)~import
alias map from Pass~1; (3)~attribute access on an imported name;
(4)~\texttt{self.method} resolved against the enclosing class's method set;
(5)~unresolved \texttt{sym:} stub. The compiler emits a \texttt{CALLS} edge
from the enclosing function or method node to the resolved destination node or
stub, recording the call-site line number and callee expression as edge evidence.

\subsubsection{VI.C\quad Third Pass: Data-Flow Extraction}

The third pass instantiates a scope-tracking \texttt{PyCodeKGVisitor} and
invokes it on the full module tree. Before the main visit, the visitor performs
a \emph{property pre-scan}: it walks the tree to register every
\texttt{@property}-decorated method by short name, so that subsequent attribute
accesses of the form \texttt{obj.attr} can emit \texttt{CALLS} edges to the
corresponding property method even when the access precedes the definition in
source order.

During the main visit, the visitor emits:

\texttt{READS} edges for every \texttt{ast.Name} node in a \texttt{Load}
context within an assignment right-hand side, call argument, or keyword argument
value, recording the variable as a data-flow source within the enclosing scope.

\texttt{WRITES} edges for every \texttt{ast.Name} node in a \texttt{Store}
context, recording the assignment target within the enclosing scope.

\texttt{ATTR\_ACCESS} edges for every \texttt{ast.Attribute} node whose object
is a simple \texttt{ast.Name}, connecting the object's variable node to the
accessed attribute node within the current scope.

\texttt{CALLS} edges from the enclosing function to registered \texttt{@property}
method nodes when an \texttt{ast.Attribute} access matches a property name
discovered in the pre-scan.

Nodes and edges produced by Pass~3 are merged into the graph using
\texttt{setdefault} semantics: Pass~1 and Pass~2 nodes and edges are
authoritative for any coincident \texttt{CONTAINS} or \texttt{CALLS} entries.

\subsubsection{VI.D\quad Post-Build Symbol Resolution}

After all three passes have been applied to all files in the repository, a
post-build symbol resolution step is executed once against the compiled graph
storage layer. This step queries all \texttt{sym:} stub nodes and all
non-symbol definition nodes from the relational database, constructs a name-to-
definitions index, and for each stub whose short name matches one or more
first-party definition nodes emits a \texttt{RESOLVES\_TO} edge from the stub to
each matching definition. Unresolved stubs remain as \texttt{symbol}-kind nodes
and are available for query but do not participate in \texttt{RESOLVES\_TO}
traversal. The operation is idempotent: duplicate edges are silently ignored on
repeated invocation.

This post-build step is analogous to the linker phase in a compiled language
toolchain~\cite{aho2006compilers}: the three AST passes are the compiler proper,
emitting relocatable object code (node and edge records with unresolved external
references as stubs); the symbol resolution step is the linker, resolving
external references to their definitions.

\subsubsection{VI.E\quad Determinism and Reproducibility}

The three-pass AST compiler is deterministic: for any given \texttt{.py} file
and Python version, it produces the same node records and edge records on every
...
source domains---including, without limitation, code, documentation, protein
structure, legal statutes, diary corpora, biochemical pathways, and scripture
or verse corpora---to participate in a single retrieval interface.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig8.png}
  \caption{The multi-domain compiler registry. Representative domain-specific
    compilers (code, documentation, protein structure, legal statutes, diary
    corpora, and biochemical pathways) emit knowledge graphs conforming to a
    uniform node and edge interface, queryable through a single federated
    retrieval engine. Additional domain compilers conforming to the same
    interface may be registered without modification to the retrieval layer.}
  \label{fig:8}
\end{figure}
...
identifiers scoped to a natural person, aggregating all knowledge graphs relevant
to that individual---code, documentation, diary, correspondence, and other
personal corpora---into a single scoped query target. The registry record may
optionally store identifying metadata associated with the person.
...
In alternative embodiments, the three-pass AST compiler for Python source code
...
artifact comprises:
\begin{itemize}[leftmargin=2.5em]
  \item a \emph{first pass} that traverses the top-level body of each source
    file's abstract syntax tree, extracts module, class, function, and method
    definition nodes, emits \texttt{CONTAINS} edges from parent nodes to their
    contained children, \texttt{IMPORTS} edges from module nodes to referenced
    module names and symbols, \texttt{INHERITS} edges from class nodes to their
    base class names, and builds an import alias map extended by a supplemental
    full-tree walk to capture imports inside function bodies;
  \item a \emph{second pass} that constructs a parent-node map via
    \texttt{ast.walk} and visits every \texttt{Call} node, resolving each callee
    to a first-party definition node or a \texttt{sym:} stub via the import
    alias map and module-local name registry, and emitting \texttt{CALLS} edges;
  \item a \emph{third pass} that runs a scope-tracking visitor including a
    pre-scan for \texttt{@property}-decorated methods, then emits \texttt{READS}
    and \texttt{WRITES} edges for variable loads and stores,
    \texttt{ATTR\_ACCESS} edges for attribute accesses on known objects, and
    \texttt{CALLS} edges for accesses to registered property methods; and
  \item a \emph{post-build symbol resolution step} that matches \texttt{sym:}
    stub nodes to first-party definition nodes by name and emits
    \texttt{RESOLVES\_TO} edges, leaving unresolvable stubs as
    \texttt{symbol}-kind nodes;
  \item whereby the compiled call and data-flow graph is a deterministic theorem
    derived from the abstract syntax tree rather than a probabilistic inference.
\end{itemize}
...
\begin{itemize}[leftmargin=2.5em]
  \item performing a \emph{first pass} over the top-level body of each source
    file that visits \texttt{ClassDef}, \texttt{FunctionDef},
    \texttt{AsyncFunctionDef}, \texttt{Import}, and \texttt{ImportFrom} AST
    node types, emitting module, class, function, and method node records with
    \texttt{CONTAINS}, \texttt{IMPORTS}, and \texttt{INHERITS} edges, and
    building an import alias map supplemented by a full-tree walk to capture
    imports inside function bodies;
  \item performing a \emph{second pass} that constructs a parent-node map over
    the full abstract syntax tree and visits every \texttt{Call} node, resolving
    each callee via the import alias map and module-local name registry and
    emitting \texttt{CALLS} edges to resolved nodes or \texttt{sym:} stubs;
  \item performing a \emph{third pass} via a scope-tracking visitor that
    pre-scans for \texttt{@property} methods and then emits \texttt{READS},
    \texttt{WRITES}, and \texttt{ATTR\_ACCESS} edges for data-flow within
    function and method bodies, and \texttt{CALLS} edges for property accesses;
    and
  \item performing a \emph{post-build symbol resolution step} that matches
    \texttt{sym:} stub nodes to first-party definition nodes by name and emits
    \texttt{RESOLVES\_TO} edges across the fully assembled graph.
\end{itemize}
...
compiled knowledge graphs through a uniform adapter interface. Representative
embodiments include a 78-work literary corpus (445,486 nodes; 4,525,716 edges;
build time 4--30 seconds per work on commodity hardware) and a decade-spanning
diary corpus of 7,282 entries compiled in approximately three minutes, each
subsequently queryable in bounded milliseconds.

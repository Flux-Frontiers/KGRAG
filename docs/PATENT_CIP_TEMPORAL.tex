...
% Graph Construction from Timestamped Text Corpora."
% =============================================================================
...
Pre-Computation and Knowledge Graph Construction from Timestamped Text Corpora}}
...
reference. The parent application discloses and claims a system and method for
offline semantic pre-computation of timestamped text corpora and construction of
a hybrid knowledge graph therefrom.
...
augmentation to arbitrary navigable axes beyond time.
...
The parent application (incorporated herein by reference) describes a system
that pre-computes 768-dimensional embedding vectors for every chunk in a
timestamped corpus and stores them in an aligned JSON cache. The present
invention builds upon this cache to create a navigable temporally-augmented
space.
...
\begin{claim}
The method of Claim~1, wherein the scalar time values $\mathbf{t}$ are
derived from ISO-format timestamps that were written directly to structured
document metadata during the corpus emission stage of the pipeline of
\parentapp, without any language model extraction, ensuring 100\% temporal
reliability regardless of the language model's size or temporal reasoning
capability.
\end{claim}

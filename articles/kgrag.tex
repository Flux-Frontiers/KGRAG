...
The \texttt{snapshot} method captures a point-in-time record of the KG's state,
returning at minimum a version string, an ISO~8601 UTC timestamp, and integer
node and edge counts. The \texttt{\_graph\_stats} helper is a concrete method on
the base class that strips all domain-specific keys from \texttt{stats()} and
normalises counts to integers; the orchestrator calls it to aggregate graph-size
totals across heterogeneous KG kinds without being confused by domain-specific
fields such as \texttt{reaction\_count} or \texttt{chunk\_count}.
...
\begin{enumerate}
  \item \emph{How has the Pepys corpus changed since last week?} \\
    ...
    the corpus, and the change in semantic coverage score. This is \texttt{kgrag
    snapshot diff} operating on two snapshot keys.

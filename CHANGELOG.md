...
- **SGSI brand identity** — `assets/sgsi_logo.png` and `assets/sgsi_logo.svg`,
  ...
  (geometry-as-axiom) and a translucent foundation pedestal beneath the
  brain (the mind grounded on derived structure). Platinum + structural-navy
  palette distinguishes the field from any single product. Full generation
  prompt and color-palette entry added to `assets/brands.md`.
- **`docs/STOICS_VS_RUSSIANS.md`** — recorded live KGRAG run answering the
  manifesto's central question (*"How do the Stoics and Russian novelists
  differ in their understanding of suffering and redemption?"*) against the
  actual Project Gutenberg texts. Every passage carries full source
  provenance — Marcus Aurelius, Dostoevsky, Tolstoy, Nietzsche — quoted
  verbatim from KGRAG output. Linked from §3 of `docs/MANIFESTO.md` so the
  thought experiment now points at a real demonstration.
- **`scripts/process_logo.py`** — logo processing pipeline: removes
  baked-in checkerboard backgrounds via connected-component flood fill +
  feathered-edge alpha, generates size variants (32 / 64 / 128 / 256 / 512
  px logos and 20 / 40 / 80 / 200 px badges), and exports an SVG (true
  vector via `vtracer` when installed, base64-embedded fallback otherwise).
- **Badge asset suite** — `assets/badges/badge_{20,40,80,200}.png` for
  social/favicon use, generated by the new processing script.
...
- **Logo asset reorganization** — flat `assets/logo_*.png` files moved into
  `assets/logos/` subdirectory for clarity; the 1024 master lives directly
  under `assets/` as `kgrag_logo_1024x1024.png` (PNG) and `.svg` (vector
  source). Sizes shipped: 32 / 64 / 128 / 256 / 512 px.
...
- `.mcp.json` — renamed `codekg` server entry to `pycodekg`; updated command from
  `/path/to/codekg mcp` to `/path/to/pycodekg-mcp --repo <path>`, matching the
  renamed binary in PyCodeKG ≥ 0.14.0.

### Added — Flux Frontiers Organization README (2026-04-22)

- `docs/FLUX_FRONTIERS_ORG_README.md` — top-level organization overview for
  Flux Frontiers: describes the full KG stack (PyCodeKG, DocKG, MetaboKG,
  DiaryKG, MemoryKG, AgentKG, FTreeKG), the `KGAdapter` protocol, MCP
  federation architecture, design principles, and research vision. Intended
  for use as the GitHub organization README.

### Added — PyCodeKG Architectural Analysis 2026-04-22 (2026-04-22)

- `analysis/kgrag_analysis_20260422.md` — Full-stack PyCodeKG analysis report
  (grade B, 80/100): 4,228 nodes / 5,678 edges (333 meaningful), fan-in/fan-out
  ranking, module coupling matrix, SIR (structural importance ranking), docstring
  coverage (79.9% overall), key call chains, and prioritized recommendations.
  Generated by `pycodekg analyze` at commit `bf1d7f6`.
